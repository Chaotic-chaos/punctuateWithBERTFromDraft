...
from model import PuncWithBERTandWav2vec
...
parser.add_argument("--test-set", default="../dataset/LibriTTS/processed_for_new/pickled/test-clean", help="test dataset path")
# parser.add_argument("--test-audio-prefix", default=r"H:\Datasets\Opensource\LibriTTS\resample\test-clean", help="Path of the audio files")
# parser.add_argument("--test-text-align-prefix", default="../dataset/LibriTTS/processed_for_new/split_text", help="Path of the align text files")
# parser.add_argument("--label-vocab", default="../dataset/LibriTTS/processed_for_new/label.dict.tsv", help="label vocabulary path")
parser.add_argument("--wav2vec-path", default="./wav2vec_large.pt", help="where the wav2vec model saved")
parser.add_argument("--label-size", default=5, help="label dimension")
# parser.add_argument("--batch-size", default=2, help="batch size")
...
if __name__ == '__main__':
    ...
    print("Loading Data...")
    # test_dataset = PunctuationDataset(input_path=args.test_set, label_vocab_path=args.label_vocab, audio_path_prefix=args.test_audio_prefix, text_split_path_prefix=args.test_text_align_prefix)
    # test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, collate_fn=collate_fn)
    with open(args.test_set, "rb") as test_set:
        test_dataloader = pickle.load(test_set)
    test_dataloader = zip(test_dataloader["audios"], test_dataloader["sentences"], test_dataloader["labels"])


    # build model
    print("Building Model...")
    model = PuncWithBERTandWav2vec(args.label_size, device, args.wav2vec_path)
    ...
    labels_all = []
    preds_all = []
    for audios, sentences, labels in tqdm(test_dataloader, desc="[Testing]"):
        # move data to device
    ...
    with open(args.test_set, "r", encoding="utf-8") as t:
        lines = [line.strip().split("\t")[1] for line in t.readlines()]
    # read vocab
